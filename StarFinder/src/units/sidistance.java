...
public class sidistance  extends siunit{
	public final static String NAME = "meters";
	public final static char SYMBOL = 'm';
	
	private sci value;
	private SI power;
	
	public sidistance(double v,SI p){
		power = p;
		value = new sci(v, power.getBase());
	}

	public sidistance(double d) {
		if(d == 0) {
			value = sci.ZERO;
			power = SI.BASE;
			return;
	...
	public boolean greaterThan(sidistance comp) {
		if(comp == null) {
			return false;
		}
	...
	public boolean lessThan(sidistance comp) {
		if(comp == null) {
			return false;
		}
		if(comp.getMyPow() > this.getMyPow()) {
	...
	public boolean equalTo(sidistance comp) {
		if(comp == null) {
			return false;
		}
		if(comp.getMyPow() > this.getMyPow()) {
	...
	public boolean lessOrEqual(sidistance comp) {
		if(comp == null) {
			return false;
		}
	...
	public boolean greaterOrEqual(sidistance comp) {
		if(comp == null) {
			return false;
		}
	...
	public sidistance plus(sidistance addend) {
		...
		return new sidistance(num,value);
	}
	
	public sidistance minus(sidistance subtrahend) {
		return this.plus(subtrahend.scale(-1));
	}

	public sidistance scale(double s) {
		if(s == 0 || this.equalTo(new sidistance(0))) {
			return new sidistance(0,SI.BASE);
		}

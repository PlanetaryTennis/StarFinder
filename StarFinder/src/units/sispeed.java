...
public class sispeed  extends siunit{
	public final static String NAME = "meters per second";
	public final static String SYMBOL = "m/s";
	
	private sci value;
	private SI power;
	
	public sispeed(double v,SI p){
		power = p;
		value = new sci(v, power.getBase());
	}

	public sispeed(double d) {
		if(d == 0) {
			value = sci.ZERO;
			power = SI.BASE;
			return;
	...
	public boolean greaterThan(sispeed comp) {
	...
	public boolean lessThan(sispeed comp) {
		if(comp.getMyPow() > this.getMyPow()) {
			return true;
		}else if(comp.getMyPow() < this.getMyPow()) {
			return false;
	...
	public boolean equalTo(sispeed comp) {
		if(comp.getMyPow() > this.getMyPow()) {
			return false;
		}else if(comp.getMyPow() < this.getMyPow()) {
			return false;
	...
	public boolean lessOrEqual(sispeed comp) {
		return !greaterThan(comp);
	}
	
	public boolean greaterOrEqual(sispeed comp) {
		return !lessThan(comp);
	}
	
	public sispeed plus(sispeed addend) {
		...
		return new sispeed(num,value);
	}
	
	public sispeed minus(sispeed subtrahend) {
		return this.plus(subtrahend.scale(-1));
	}

	public sispeed scale(double s) {
		if(s == 0||this.getMyNum() == 0) {
			return new sispeed(0,SI.BASE);
		}
		int pow = this.getMyPow();

	...
	public void GeneratePrimeNetwork() {
		...
		for(int i = 0;i < myGalaxy.getMySectors().size();i++) {
			...
			if(i < myGalaxy.getMySectors().size()-1) {
				myPrimes.add(PrimaryGate.randomPrime(myGalaxy.getMySectors().get(i+1).getRegions().get(random.nextInt(myGalaxy.getMySectors().get(i+1).getRegions().size()))));
			}else {
				myPrimes.add(PrimaryGate.randomPrime(myGalaxy.getMySectors().get(0).getRegions().get(random.nextInt(myGalaxy.getMySectors().get(0).getRegions().size()))));
			}
		...
		for(int i = 0;i < myGalaxy.getMySectors().size();i++)
			for(int j = 0;j < myGalaxy.getMySectors().get(i).getRegions().size();j++) {
				...
				myPrimes.add(PrimaryGate.randomPrime(myGalaxy.getMySectors().get(i).getRegions().get(random.nextInt(myGalaxy.getMySectors().get(i).getRegions().size()))));
